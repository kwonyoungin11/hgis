...
static QString northArrowSvg() {
  const QString root = qEnvironmentVariable("OSGEO4W_ROOT");
  const QStringList cands = {
    root + QStringLiteral("/apps/qgis-dev/svg/arrows/NorthArrow_02.svg"),
    QStringLiteral("C:/OSGeo4W/apps/qgis-dev/svg/arrows/NorthArrow_02.svg"),
...
QString LayoutService::createReportLayout(QgsProject* project, const QString& titleKo,
  ...
  double pageW = (paper == Paper::A3) ? 297.0 : 210.0;
  double pageH = (paper == Paper::A3) ? 420.0 : 297.0;
  ...
  const QString crsAuth = project->crs().authid().isEmpty()
                              ? QStringLiteral("EPSG:5186")
                              : project->crs().authid();
  const QString sheet = titleKo.isEmpty() ? QStringLiteral("조사도면") : titleKo;

  const double margin = 10.0;
  const double sideW = 42.0;
  const double mapW = pageW - margin * 2 - sideW - 4.0;
  const double mapH = pageH - margin * 2 - 30.0;
  ...
  auto* meta = new QgsLayoutItemLabel(layout);
  meta->setText(QStringLiteral("CRS %1  |  방위: 진북  |  단위: m").arg(crsAuth));
  meta->attemptSetSceneRect(QRectF(margin, 13, mapW, 6));
  meta->setFont(QFont(QStringLiteral("Malgun Gothic"), 8));
  layout->addLayoutItem(meta);

  auto* map = new QgsLayoutItemMap(layout);
  map->attemptSetSceneRect(QRectF(margin, 22, mapW, mapH));
  map->setFrameEnabled(true);
  map->setCrs(project->crs());
  map->setExtent(ext);
  map->setLayers(allMapLayers(project));
  ...
  if (QFile::exists(svg)) {
    ...
    north->setLinkedMap(map);
    north->setNorthMode(QgsLayoutItemPicture::TrueNorth);
    north->attemptSetSceneRect(QRectF(sideX, 22, 28, 28));
    layout->addLayoutItem(north);
  } else {
    auto* north = new QgsLayoutItemLabel(layout);
    north->setText(QStringLiteral("N\n↑\n진북"));
    north->setHAlign(Qt::AlignHCenter);
    north->attemptSetSceneRect(QRectF(sideX, 22, 28, 24));
    north->setFont(QFont(QStringLiteral("Malgun Gothic"), 11, QFont::Bold));
    north->setFrameEnabled(true);
    layout->addLayoutItem(north);
  }
  ...
  sb->setNumberOfSegments(4);
  sb->applyDefaultSize(Qgis::DistanceUnit::Meters);
  sb->attemptSetSceneRect(QRectF(margin, pageH - 18, 90, 12));
  layout->addLayoutItem(sb);
  ...
  scaleLbl->setText(sc > 0 ? QStringLiteral("축척 1 : %1").arg(sc)
                           : QStringLiteral("축척: 축척자 참조"));
  scaleLbl->setId(QStringLiteral("scale_label"));
  scaleLbl->attemptSetSceneRect(QRectF(margin + 95, pageH - 16, 60, 8));
  scaleLbl->setFont(QFont(QStringLiteral("Malgun Gothic"), 9, QFont::Bold));
  layout->addLayoutItem(scaleLbl);

  auto* crsLbl = new QgsLayoutItemLabel(layout);
  crsLbl->setText(QStringLiteral("좌표계: %1").arg(crsAuth));
  crsLbl->attemptSetSceneRect(QRectF(sideX, 58, sideW, 10));
