...
QIcon icon(const QString& id) {
  ...
  else if (id == QLatin1String("map")) ic = drawMap();
  else if (id == QLatin1String("satellite")) ic = drawSatellite();
  else if (id == QLatin1String("polygon")) ic = drawPolygon();
  else if (id == QLatin1String("line")) ic = drawLine();
...
QIcon step(int oneBasedIndex) {
  ...
  const int i = qBound(0, oneBasedIndex - 1, 6);
  return drawStep(oneBasedIndex, colors[i]);
}
