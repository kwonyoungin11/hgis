...
KaLayerOpacityRail::KaLayerOpacityRail(QWidget* host) : QFrame(host), m_host(host) {
  ...
  opCol->setContentsMargins(0, 0, 0, 0);
  opCol->setSpacing(4);
  auto* brCol = new QVBoxLayout();
  brCol->setContentsMargins(0, 0, 0, 0);
  brCol->setSpacing(4);
  ...
  m_brightTitle = new QLabel(QStringLiteral("밝\n기"), this);
  m_brightTitle->setObjectName(QStringLiteral("layerOpacityTitle"));
  m_brightTitle->setAlignment(Qt::AlignHCenter | Qt::AlignTop);
  m_brightTitle->setWordWrap(false);

  m_bright = new QSlider(Qt::Vertical, this);
  ...
  m_brightValue->setAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
  m_brightValue->setMinimumWidth(32);
  ...
  opCol->addWidget(m_title, 0, Qt::AlignHCenter);
  opCol->addWidget(m_slider, 1, Qt::AlignHCenter);
  opCol->addWidget(m_value, 0, Qt::AlignHCenter);
  brCol->addWidget(m_brightTitle, 0, Qt::AlignHCenter);
  brCol->addWidget(m_bright, 1, Qt::AlignHCenter);
  brCol->addWidget(m_brightValue, 0, Qt::AlignHCenter);
  lay->addLayout(opCol, 1);
  lay->addLayout(brCol, 1);
