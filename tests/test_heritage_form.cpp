...
private slots:
  ...
  void queryIsPercentEncodedExactlyOnce() {
    // 2026-09-13: QUrlQuery/setQuery 를 거치면 Qt 가 인코딩을 다시 풀어
    // 한글·공백이 날것으로 나갔다. 문자열을 직접 만들어야 한 번만 인코딩된다.
    QByteArray query;
    const QList<QPair<QString, QString>> pairs = {
        {QStringLiteral("codedetaCd"), QStringLiteral("510000,ADDR510000")},
        {QStringLiteral("searchParams"), QStringLiteral("지역:강원특별자치도 강릉시 / ")},
    };
    for (const auto& p : pairs) {
      if (!query.isEmpty()) query += '&';
      query += QUrl::toPercentEncoding(p.first);
      query += '=';
      query += QUrl::toPercentEncoding(p.second);
    }
    const QUrl url = QUrl::fromEncoded(
        "https://intranet.gis-heritage.go.kr/a.do?" + query, QUrl::StrictMode);
    const QByteArray out = url.toEncoded();
    QVERIFY2(!out.contains(' '), out.constData());          // 공백이 날것으로 남지 않는다
    QVERIFY2(!out.contains("%25"), out.constData());        // 두 번 인코딩되지 않는다
    QVERIFY(out.contains("510000%2CADDR510000"));           // 쉼표는 인코딩된다
    QVERIFY(out.contains("%EC%A7%80%EC%97%AD"));            // 한글은 UTF-8 로 인코딩된다
  }
