...
This directory is the **Grok-native** copy (`.grok/skills/unlazy`). Follow [SKILL.md](SKILL.md). The Stop hook is already registered in `.grok/hooks/ka-hgis.json`. **Do not run `install-hooks.mjs` here** (that installer writes Claude Code `.claude/settings`).

## Install

Upstream install (other agents only) uses the [skills CLI](https://github.com/vercel-labs/skills):
...
In **this repo**, Grok Stop is already wired (`.grok/hooks/ka-hgis.json` → `unlazy-stop.ps1`). It arms only for a valid `.unlazy/<scope>/` pipeline. **Do not run `install-hooks.mjs` here** — that installer writes Claude Code `.claude/settings*.json`.

The hook scans the resolved ledger and returns `{"decision":"block","reason":"..."}` while gates remain unmet. It does not execute checks. Its session-keyed progress guard releases after six consecutive blocks without ledger progress. Keep `.unlazy/` and `.unlazy-hook-state.json` in the project's ignore rules.
